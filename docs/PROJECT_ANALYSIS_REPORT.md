...
数值分类：本文架构/安全/质量分数和星级是无量表支持的历史主观估计；代码规模统计未绑定可复现命令和提交，也仅属历史估计。它们不是测得结果或当前验证结论，不能用于申请或生产等级声明。
...
撤下原“当前值”（包括约 2.5 秒首屏）及通过标记：没有提供可复现测量。下面保留历史设计目标，不代表达到目标或当前 SLA。

| 指标 | 历史设计目标 | 验证状态 |
|---|---|---|
| API P95 响应时间 | <100ms | 尚未基准测试 |
| SignalR 推送延迟 | <50ms | 尚未基准测试 |
| 数据库写入 P95 | <20ms | 尚未基准测试 |
| 页面首次加载 | <2s | 尚未基准测试 |
...
#### 测试覆盖
- [ ] 历史设计目标：单元测试覆盖率达到 60%；没有覆盖率报告证明已达到目标。
- [ ] 集成测试完善
...
原表混合了未绑定提交的代码量估计与主观评分，且没有可复现统计方法，数值比较已撤下。当前规模需对指定提交运行统计；当前验证边界见 FEATURE_MATRIX.md 和 OSS_READINESS_REPORT.md。
...
历史版本包含遥测、告警及启发式分析实现；没有证据证明生产级安全、AI 预测准确性或现场维护收益。
...
撤下无量表支持的数值评分、目标差值，以及“一至两周后提高到某分数”的推断。软件修改必须由具体测试和审查证据评价，不能换算成安全认证分数。

### 建议
按可复现问题和影响排序，优先修复凭据泄露与授权问题；没有承诺修复时长或评分提升。
