...
public sealed class EdgeConfigRepository : IEdgeConfigRepository
{
    ...
    public EdgeConfigRepository(INpgsqlConnectionFactory factory, ILogger<EdgeConfigRepository> logger)
    {
        _factory = factory;
        _logger = logger;
    }
    ...
    public async Task<IReadOnlyList<EdgeSummaryDto>> ListAllAsync(CancellationToken ct)
    {
        const string sql = @"
            SELECT
                ...
                COALESCE(es.is_online, false) AS ""IsOnline"",
    ...
    public async Task UpsertAsync(EdgeConfigDto config, CancellationToken ct)
    {
        ...
        await conn.ExecuteAsync(new CommandDefinition(sql, new
        {
            ...
            config.Name,
            config.Description,
            ProcessingEnabled = config.Processing.Enabled,
            ...
            OutlierEnabled = config.Processing.OutlierEnabled,
            config.Processing.OutlierSigmaThreshold,
            config.Processing.OutlierAction,
            StoreForwardEnabled = config.StoreForward.Enabled,
            config.StoreForward.MaxStoreSizeMB,
            config.StoreForward.RetentionDays,
            CompressionEnabled = config.StoreForward.CompressionEnabled,
...
public sealed class TagProcessingConfigRepository : ITagProcessingConfigRepository
{
    ...
    public TagProcessingConfigRepository(INpgsqlConnectionFactory factory, ILogger<TagProcessingConfigRepository> logger)
    {
        _factory = factory;
        _logger = logger;
    }
    ...
    public async Task<PagedTagConfigResult> ListByEdgeAsync(string edgeId, int page, int pageSize, string? search, CancellationToken ct)
    {
        ...
        {
            whereClause += " AND tag_id ILIKE @Search";
        }
...
public sealed class EdgeStatusRepository : IEdgeStatusRepository
{
    ...
    public EdgeStatusRepository(INpgsqlConnectionFactory factory, ILogger<EdgeStatusRepository> logger)
    {
        _factory = factory;
        _logger = logger;
    }
